    ...
    int       nCmdShow)
{
    ...
    return JumpbirdPlayer::createAndRun();
}

int JumpbirdPlayer::createAndRun(void)
{
    JumpbirdPlayer *host = JumpbirdPlayer::sharedInstance();
    int ret = host->run();
    JumpbirdPlayer::purgeSharedInstance();
    return ret;
}

JumpbirdPlayer *JumpbirdPlayer::s_sharedInstance = NULL;

JumpbirdPlayer *JumpbirdPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new JumpbirdPlayer();
    }
    return s_sharedInstance;
}

void JumpbirdPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
JumpbirdPlayer::JumpbirdPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int JumpbirdPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "jumpbird-player");
...
void JumpbirdPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void JumpbirdPlayer::createViewMenu(void)
{
...
void JumpbirdPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void JumpbirdPlayer::updateWindowTitle(void)
{
}

void JumpbirdPlayer::relaunch(void)
{
...
void JumpbirdPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void JumpbirdPlayer::onFileOpenProject(void)
{
...
void JumpbirdPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"JumpbirdPlayer");
...
void JumpbirdPlayer::onFileProjectConfig(void)
{
...
void JumpbirdPlayer::onFileRelaunch(void)
{
    relaunch();
}

void JumpbirdPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void JumpbirdPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void JumpbirdPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void JumpbirdPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void JumpbirdPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT JumpbirdPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    JumpbirdPlayer *host = JumpbirdPlayer::sharedInstance();
...
INT_PTR CALLBACK JumpbirdPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
